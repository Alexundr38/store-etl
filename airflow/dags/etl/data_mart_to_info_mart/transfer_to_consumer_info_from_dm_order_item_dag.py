...
def transfer_to_consumer_info_from_dm_order_item_dag():
    ...
    @task(task_id='transfer_data_to_consumer_info')
    def transfer_data_to_consumer_info(last_time: datetime):
        dwh_hook = ClickHouseHook(clickhouse_conn_id='dwh_clickhouse_data_mart_to_info_mart')
        connection = dwh_hook.get_conn()

        insert_sql = """
            INSERT INTO info_mart.consumer_info (time_start_interval, consumer_id, total_quantity, 
                                                 total_amount, avg_amount, total_order, unique_item)
                SELECT
                    toStartOfInterval(f_oi.order_dt, INTERVAL 10 MINUTE) AS time_start_interval,
                    d_c.consumer_id,
                    ...
                    COUNT(DISTINCT f_oi.dim_item_id) AS unique_item
                FROM
    ...
    transfer_data_to_consumer_info(last_time)

transfer_to_consumer_info_from_dm_order_item_dag()

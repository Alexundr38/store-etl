...
def transfer_to_item_info_from_dm_order_item_dag():
    ...
    @task(task_id='transfer_data_to_item_info')
    def transfer_data_to_item_info(last_time: datetime):
        dwh_hook = ClickHouseHook(clickhouse_conn_id='dwh_clickhouse_data_mart_to_info_mart')
        connection = dwh_hook.get_conn()

        insert_sql = """
            INSERT INTO info_mart.item_info (time_start_interval, dim_item_id, name, total_quantity, 
                                                 total_amount, total_order, unique_consumer, unique_store)
                SELECT
                ...
                FROM
                    ...
                    JOIN dm_order_item.dim_item d_i USING (dim_item_id)
                WHERE 
                    f_oi.order_dt > %(last_time)s
                GROUP BY 
                    f_oi.dim_item_id, d_i.name, time_start_inerval
                ORDER BY 
                time_start_inerval;
        """
    ...
    transfer_data_to_item_info(last_time)

transfer_to_item_info_from_dm_order_item_dag()
